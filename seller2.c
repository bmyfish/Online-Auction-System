...
#define SELLERPORT "2228"
#define BACKLOG 10
#define MAXDATASIZE 256
#define fileid "Seller#2"
#define itemtxt "itemList2.txt"
#define logintxt "sellerPass2.txt"
#define stop "term"
...
int authentication(char *read, char *filename)
{
    ...
    /*
	 * L66 to L91 are from the given tutorial http://beej.us/guide/bgnet/.
	 */
	/* create socket */
...
int PreAuction(char *sendfile,char *sendname,int flag)
{
    ...
    /*
	 * L182 to L216 are from the given tutorial http://beej.us/guide/bgnet/.
	 */
	/* create socket */
...
int Auction(char* sendname) {
    ...
    /*
	 * L276 to L319 are from the given tutorial http://beej.us/guide/bgnet/.
	 */
	/* create socket */
...
int main(void) {
  /*  int i =0;
    ...
    char inputname[MAXDATASIZE];
    PPId = getpid();
		// Added to Kill child and parent process on Ctrl+C - End
    while(i < 2){
        switch(i){
            case 0:
                strcpy(input,"sellerPass1.txt");
                strcpy(inputname,"seller1");
                break;
            case 1:
                strcpy(input,"sellerPass2.txt");
                strcpy(inputname,"seller2");
                break;
        }
        PId[i] = fork();
        if(PId[i] == 0){
            seller(input, inputname);
            sleep(1);
            exit(0);
        }
        i++;
		...
		// Waiting for Ctrl+C to Kill all the Child Processes - Start
    while (1) {
    }*/
